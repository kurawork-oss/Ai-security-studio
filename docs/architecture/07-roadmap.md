...
- Projects CRUD / Providers(Gemini) + provider_keys(暗号化) / API Keys 発行。
- Protect Rules（既定 12 種の ON/OFF + action）。
- **Protect API** / **Analyze API（Gemini）** + フェイルクローズ。
- PII Engine（Presidio + GiNZA + JP Regex Recognizers）。
- Logs（メタデータ）+ Playground。

**Exit**: 発行キーで Protect/Analyze が動作、Playground で検証可、ログ記録、[MVP 受け入れ基準](./08-mvp.md#受け入れ基準)充足。

## Phase 2 — 運用強化（Hardening & Insights）
- Analytics ダッシュボード（時系列・種別内訳・エラー率）+ `analytics_daily` ロールアップ。
- カスタム Protect ルール（正規表現追加）。
- API キーのスコープ/失効/ローテーション、レート制限・クォータ、`Idempotency-Key`。
- 監査ログ（audit_logs）、構造化ログ強化、メトリクス/トレース。
- SDK（`@secureai/sdk`, `secureai`）α版。

**Exit**: 主要指標が可視化、レート制限稼働、監査ログ記録、SDK で 3 行統合。

## Phase 3 — マルチプロバイダー / 組織（Scale）
- Provider Adapter を拡張: Claude / OpenAI / DeepSeek / Grok / Local LLM。
...
- 高度検出（カスタム ML モデル、文脈依存検出）、ストリーミング対応、バッチ API。
